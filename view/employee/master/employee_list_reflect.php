	...
	<tbody>
		...
		while($row_emp = mysqli_fetch_assoc($sq_emp)){
			...
			if($row_emp['id_proof_url']!=""){
				...
				<td>
					<button class="btn btn-info btn-sm" onclick="display_modal(<?= $row_emp['emp_id'] ?>);btnDisableEnable(this.id)" id="display_modal_user_edit_btn<?= $row_emp['emp_id'] ?>" title="View User"><i class="fa fa-eye"></i></button>
					<button class="btn btn-info btn-sm" onclick="update_modal(<?= $row_emp['emp_id'] ?>);btnDisableEnable(this.id)" id="display_modal_user_view_btn<?= $row_emp['emp_id'] ?>" title="Edit User"><i class="fa fa-pencil-square-o"></i></button>
				</td>
				<!-- <td>
