	...
	<tbody>
		...
		while($row_terms = mysqli_fetch_assoc($sq_terms)){
			...
			<tr class="<?= $bg ?>">
				...
				<td>
					<button class="btn btn-info btn-sm" onclick="update_modal(<?= $row_terms['terms_and_conditions_id'] ?>);btnDisableEnable(this.id)" id="update_modal_terms_edit_btn<?= $row_terms['terms_and_conditions_id'] ?>" title="Update Details"><i class="fa fa-pencil-square-o"></i></button>
				</td>
			</tr>
			<?php
		}
