...
if($row_payment['amount'] != '0.00'){
		...
		if($row_payment['payment_mode'] == 'Credit Note' || ($row_payment['payment_mode'] == 'Credit Card' && $row_payment['clearance_status']=="Cleared")){
			...
			$edit_btn = '<button class="btn btn-info btn-sm" data-toggle="tooltip" onclick="update_modal('.$row_payment['payment_id'].')" title="Update Details" id="update-'.$row_payment['payment_id'].'"><i class="fa fa-pencil-square-o"></i></button>';
