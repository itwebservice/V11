...
while($row_gl = mysqli_fetch_assoc($sq_gl)){
	...
	$temp_arr = array('data'=>array(
		...
		'<a href="../finance_master/ledger_master/view/index.php?ledger_id='.$row_gl['ledger_id'].'" target="_BLANK" class="btn btn-info btn-sm" title="View Details"><i class="fa fa-eye"></i></a>
		<button class="btn btn-info btn-sm" onclick="update_modal('. $row_gl['ledger_id'] .')" title="Update Details" data-toggle="tooltip"><i class="fa fa-pencil-square-o"></i></button>'),"bg"=>$bg
	);
	if($chk_balance == 0){
		if($balance > 0)
