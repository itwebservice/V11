		...
		foreach($booking_array as $booking_array_item){
			...
			if($role== 'Admin' || $role=='Branch Admin' || $role=='Accountant'){ 
				...
				if($other_data_arr['tour_type']!="Group Tour" && $incentive_count==0 && ($role== 'Admin' || $role=='Branch Admin' || $role=='Accountant')){
					$edit='<a href="javascript:void(0)" onclick="incentive_edit_modal(\''.$other_data_arr['booking_id'] .'\',\''. $other_data_arr['emp_id'] .'\',\''.$other_data_arr['tour_type'].'\')" class="btn btn-info btn-sm" data-toggle="tooltip" title="Update Details"><i class="fa fa-pencil-square-o"></i></a>';
